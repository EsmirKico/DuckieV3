...
print("🚀 Using LM Studio for Qwen Vision Model")
...
async def autonomous_drive(file: UploadFile = File(...)):
    ...
    if vision_response is None:
        return AutonomousResponse(
            ...
            target_speed=0.0,
            driving_state="stopped",
            debug_info="LM Studio analysis failed"
        )
    ...
    cv2.putText(debug_image, "LM Studio + Qwen", 
    ...
    cv2.imshow("Autonomous Driving (LM Studio + Qwen)", debug_image)
    cv2.waitKey(1)
...
@app.get("/")
def read_root():
    return {"message": "Duckietown Autonomous Driving API with LM Studio + Qwen is running"}
...
if __name__ == "__main__":
    print("🚀 Starting Autonomous Driving Server with LM Studio + Qwen")
